...
function connect() {
    ...
    if grep -q "successfully activated" <<< $result; then
        notify-send VPN "Successfully connected to VPN RodeoFX"
        echo "1" >| ~/.vpn_status
    else
        notify-send -u critical VPN "Could not connect to VPN RodeoFX"
        echo "0" >| ~/.vpn_status
    fi
}
...
function disconnect() {
    ...
    if grep -q "successfully deactivated" <<< $result; then
        notify-send VPN "Successfully disconnected from VPN"
        echo "0" >| ~/.vpn_status
    else
        notify-send VPN "Could not disconnect VPN ?"
        echo "1" >| ~/.vpn_status
    fi
}
